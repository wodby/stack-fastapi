# FastAPI application stack for Kubernetes on Wodby

Deploy FastAPI applications on Kubernetes with Wodby.

This repository defines the Wodby stack manifests and default service
composition for FastAPI.
...
- [Python (FastAPI) service](https://github.com/wodby/service-fastapi)
...
| --- | --- |
| Python (`fastapi`)<br>`fastapi` | required; enabled by default; main service; links: `db` → `postgres`, `redis` → `valkey`, `sendmail` → `mailpit` |
...
Start from [FastAPI boilerplate](https://github.com/wodby/fastapi-boilerplate), or connect your own compatible source
repository.
